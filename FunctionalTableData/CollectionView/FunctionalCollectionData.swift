...
public class FunctionalCollectionData: NSObject {
	...
	public func scroll(to keyPath: KeyPath, animated: Bool = true, scrollPosition: UICollectionView.ScrollPosition = [UICollectionView.ScrollPosition.bottom, UICollectionView.ScrollPosition.right]) {
		guard let aCollectionView = collectionView, let indexPath = indexPathFromKeyPath(keyPath) else { return }
